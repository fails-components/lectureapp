...
export class NoteScreenBase extends Component
{
    ...
    componentDidMount() 
    {
        ...
        this.resizeeventlistener = function (event) {
            ...
            myself.setState( {bbwidth: iwidth, bbheight: iheight, devicePixelRatio: window.devicePixelRatio || 1});
        ...
        if (this.state.bbwidth!==window.innerWidth
            || this.state.bbheight!==window.innerHeight) {
            let iwidth = window.innerWidth;
            let iheight =window.innerHeight;
            console.log("resize mount"/*,myself.bgtrick*/,(iheight/iwidth) );
            this.props.updateSizes({scrollheight: (iheight/iwidth)});
             this.setState( {bbwidth:window.innerWidth,bbheight: window.innerHeight,  devicePixelRatio: window.devicePixelRatio || 1});
        }
...
/*
    ...
    setScrollOffset(scrolloffset)
    {
        ...
        if (this.blackboard && this.blackboard.current) this.blackboard.current.setScrollOffset(scrolloffset);
    ...
    calcCurpos() {
        if (!this.props.isnotepad && this.blackboard && this.blackboard.current) return this.blackboard.current.calcCurpos()/this.state.bbheight*this.state.bbwidth;
        return 0;
    }
    ...
    setHasControl(hascntrl) {
        ...
        if (this.props.isnotepad && this.blackboard && this.blackboard.current) this.blackboard.current.setblocked(!hascntrl);
    };

    reactivateToolBox() {
        console.log("reactivate Toolbox NSB",this.blackboard);
        if (this.blackboard && this.blackboard.current) this.blackboard.current.reactivateToolBox();
    };


    endButtonPressed(){
        this.props.endbuttoncallback();
    };
    ...
    receiveData(data) {
        ...
        if (this.blackboard && this.blackboard.current) this.blackboard.current.receiveData(data);
    };

    receivePictInfo(data) {
        if (this.blackboard && this.blackboard.current)  this.blackboard.current.receivePictInfo(data);
    };

    receiveBgpdfInfo(data)
    {
        if (this.blackboard && this.blackboard.current)  this.blackboard.current.receiveBgpdfInfo(data);
    }

    replaceData(data) {
        if (this.blackboard && this.blackboard.current) this.blackboard.current.replaceData(data);
    };
    ...
    receiveFoG(data) {
        if (this.blackboard && this.blackboard.current) this.blackboard.current.receiveFoG(data);
    };

    enterAddPictureMode(uuid,url) {
        if (this.blackboard && this.blackboard.current) this.blackboard.current.enterAddPictureMode(uuid,url);
    };
    ...
    onKeyDown(key) {  
        ...
        switch (key.keyCode) {
            case 0x28:  // arrowdown
            if (this.blackboard.current.scrollboardKeys) this.blackboard.current.scrollboardKeys(0,0.05);
            break;
            case 0x26:  // arrowUp
                if (this.blackboard.current.scrollboardKeys) this.blackboard.current.scrollboardKeys(0,-0.05);
            break;
       ...
       // console.log("tsw",this.state.width,this.state.height,this.blackboard);
        return <div  style={{width: this.props.width,  height: this.props.height, overflow: "hidden", position:"relative"}}>
            {this.props.showscreennumber &&
             <span style={{position: "absolute", top: "2vw" ,left: "4vw", fontSize: "10vw", textShadow: "2px 2px 8px "+this.props.screennumbercolor, color: this.props.screennumbercolor  }}
                    zIndex={50}> {this.props.screennumber+1} </span>}
              {  !this.props.isnotepad ? 
              <Blackboard ref={this.blackboard} backcolor={this.props.backgroundcolor} notepadscreen={this} bbwidth={this.state.bbwidth} bbheight={this.state.bbheight} 
              pageoffset={this.props.pageoffset*this.state.bbheight/this.state.bbwidth} pageoffsetabsolute={this.props.pageoffsetabsolute} ></Blackboard>
              :<BlackboardNotepad ref={this.blackboard}  backcolor={this.props.backgroundcolor} notepadscreen={this} bbwidth={this.state.bbwidth} bbheight={this.state.bbheight}
              devicePixelRatio={this.state.devicePixelRatio} ></BlackboardNotepad> }
             { this.props.isnotepad && <ToolBox ref={this.toolbox} bbwidth={this.state.bbwidth} bbheight={this.state.bbheight} notepad={this} dispres={this.dispres}/>}
             { this.props.isnotepad && <ConfirmBox ref={this.confirmbox } bbwidth={this.state.bbwidth} bbheight={this.state.bbheight} notepad={this} />}
             </div>
    };
